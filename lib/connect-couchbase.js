...
module.exports = function(session){
        ...
        if ( typeof(connectOptions.db) != 'undefined' ) {
            ...
            Couchbase.Cluster.connect(
              connectOptions.host,
              {
                ...
                // disconnect
                this.client.disconnect = cluster.close;
                self.emit('connect');
            }).catch((err) => {
    ...
    CouchbaseStore.prototype.get = function(sid, fn){
        ...
        sid = this.prefix + sid;
        debug('GET "%s"', sid);
        this.client.get(sid, { timeout: this.operationTimeout }, function(err, data){
    ...
    CouchbaseStore.prototype.set = function(sid, sess, fn){
        ...
        try {
            ...
            this.client.upsert(sid, sess, { expiry: ttl, timeout: this.operationTimeout }, function(err){
    ...
    CouchbaseStore.prototype.all = function(fn){
        this.client.query(this.queryAll, [], { readonly: true, timeout: this.operationTimeout }, function(err, result) {
            fn(err, result.rows);
        });
    };
    ...
    CouchbaseStore.prototype.destroy = function(sid, fn){
        if ('function' !== typeof fn) { fn = noop; }
        sid = this.prefix + sid;
        this.client.remove(sid, { timeout: this.operationTimeout }, fn);
    };
    ...
    CouchbaseStore.prototype.touch = function (sid, sess, fn) {
        ...
        this.client.touch(this.prefix + sid, ttl, { timeout: this.operationTimeout }, fn);
    };
